// MyWiki knowledge-base indicator: a compact "mywiki" label plus a colour-coded dot —
// green (connected), red (not connected), amber (still probing / unknown). It
// doubles as a link whose target depends on whether mywiki is configured:
//   - configured    -> the host's `/plugins` page (manage / configure it),
//   - not configured -> the mywiki plugin repo,
...
const REPO_URL = "https://github.com/bitfrost7/mywiki";
...
function labelFor(connected: boolean | null): string {
  if (connected === null) return "MyWiki: checking…";
  return connected ? "MyWiki: connected" : "MyWiki: not configured";
}
